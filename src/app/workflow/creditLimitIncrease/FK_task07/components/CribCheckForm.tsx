...
const UserForm: FC<UserFormProps> = (props) => {
  ...
  const createDayJs = (year: number, month: number) => {
    const date = dayjs()
      .set("year", year)
      .set("month", month - 1);
  ...
  useEffect(() => {
    if (cliProcessData != null) {
      // alert(cliProcessData.cribNicNumber)
      setValue("cribNicNumber", cliProcessData.cribNicNumber);
      setValue("contactNumber", cliProcessData.contactNumber);
      setValue("cribAccountNumber", cliProcessData.cribAccountNumber);
      setValue("customer", cliProcessData.customer);
      setValue("employment", cliProcessData.employment);
      setValue(
        "yearMonth",
      ...
      setValue("relationshipDuration", cliProcessData.relationshipDuration);
    }
  }, [cliProcessData]);
