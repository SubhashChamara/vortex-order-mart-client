...
const FK_Task16: FC<FK_Task16Props> = (props) => {
  const { task } = props;
  ...
  const schema = z.object({
    ...
    daHoldersApproval: z.boolean().optional().nullable(),
    customerCategory: z.boolean().optional().nullable(),
    isCustomersRequested: z.boolean().optional().nullable(),
    // enhancementTypeUW: z
    //   .object({ id: z.number(), name: z.string() })
    //   .nullable()
    //   .refine((val) => val !== null, {
  ...
  const handleOnSubmit = async (formData: FormType) => {
    ...
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.saveDocCheckList(request)
    );

    if (err === null) {
      toast.success("Successfully Saved Doc Check Details");
    } else {
      toast.error(err.msg);
    }
    /**
        ...
        >
          ...
          <div className="mt-6">
            <Accordion defaultExpanded>
              ...
              <AccordionDetails>
                <CBODocCheckForm task={task} cliProcessData={cliProcessData} />
              </AccordionDetails>
            </Accordion>
          </div> */}
