...
const FK_Task01: FC<FK_Task01Props> = (props) => {
  ...
  const [selectedUserRole, setSelectedUserRole] = useState<UserRoleInfo | null>(null)
  const leftChecked = intersection(checked, left);
  const rightChecked = intersection(checked, right);


  const fetchRightList = async (roleId: string, newAllocatedList: GroupInfo[]) => {
    ...
    if (data !== null) {
      newAllocatedList.forEach(newRightItem => {
        if (!data.some((dataItem: GroupInfo) => dataItem.id === newRightItem.id)) {
  ...
  const fetchLeftList = async (roleId: string, allGroups: GroupInfo[], right: GroupInfo[]) => {
    let newUnallocatedList = filterGroups(allGroups, right);
    const { data, err } = await Api.performRequest((r) =>
    ...
    if (data !== null) {
      newUnallocatedList.forEach(newLeftItem => {
        if (!data.some((dataItem: GroupInfo) => dataItem.id === newLeftItem.id)) {
  ...
  const customList = (title: string, items: GroupInfo[]) => (
    <>
      ...
      <Paper className='min-h-screen' sx={{ width: '32vw', height: 230, overflow: 'auto', mt: 2 }} >
          ...
          {items.map((value: GroupInfo) => {
            ...
            return (
              <ListItemButton
                ...
                role="listitem"
                aria-disabled
                className={value.isNew ? 'bg-gray-700 border-4 border-black m-1 text-white' : 'bg-white m-1'}
              >
