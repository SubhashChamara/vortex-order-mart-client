...
const OverDraftFlow: React.FC<OverDraftFlowProps> = ({ task }) => {
  ...
  const methods = useForm<FormType>({
    ...
    resolver: zodResolver(getFormSchema(step)),
  });

  const { setValue, handleSubmit, watch, formState } = methods;
  ...
  const handleUnderwriterChecklistSubmission = async () => {
    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveUWChecklist(underwriterChecklist)
    );

    if (!err) {
      toast.success("Underwriter checklist saved");
    } else {
      toast.error(err.msg);
    }
  };
