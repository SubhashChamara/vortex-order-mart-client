...
import ODEODReportTable from "./components/ODEODReportTable";
...
const ODEODReport = () => {
  ...
  const [ODEODReportList, setODEODReportList] =
    useState<Pageable<ODEODReportIf> | null>(null);
  ...
  const handleFetchCliReportList = useCallback(async () => {
    ...
    const { data, err } = await Api.performRequest((r) =>
      r.reports.getODEODReport(startDate, endDate, statusId, size, page)
    );
    Logger.debug(
  ...
  return (
    <div className="px-12 pb-12">
      ...
      {loading ? (
        ...
        <>
          <ODEODReportTable
            ODEODReportList={ODEODReportList || null}
            setPage={setPage}
            page={page}
          />
        </>
