...
import { customerVerificationSchema } from "../schema/Schema";
...
const OverDraftFlow: React.FC<OverDraftFlowProps> = ({ task }) => {
  ...
  const handleCustomerVerificationSubmission = async (formData: FormType) => {
    const request: CustomerVerificationRequest = {
      contactName: formData.contactPerson,
      address: formData.isAddressVerified,
      nic: formData.isNicVerified,
      dob: formData.isDobVerified,
      splComment: formData.specialVerificationComment,
    };

    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveCustomerVerify(task.processInstanceId, request)
    );
    ...
    },
    {
      ...
      content: (
        <form
          ...
          onSubmit={handleSubmit(handleCustomerVerificationSubmission)}
        >
          <div className="grid grid-cols-2 gap-14">
            <CustomerVerification editable={true} />
            <AttachmentsForm
              documentList={documentList}
              selectedDocument={selectedDocument}
              setSelectedDocument={setSelectedDocument}
