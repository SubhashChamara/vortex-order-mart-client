...
import { documentCheckSchema } from "../schema/Schema";
...
const OverDraftFlow: React.FC<OverDraftFlowProps> = ({ task }) => {
  ...
  const handleDocCheckSubmission = async (formData: FormType) => {
    const request: OdProcessRequest = {
      ...
      termLoanFacilityType:
        processInfo?.facilityType == "TERM_LOAN"
          ...
          : null,
      noCribHistory: formData.isNoCribHistory ? true : false,
      fcyBackedOd: formData.isFCYBackedOD ? true : false,
      tbillBackedOd: formData.isTBillBackedOD ? true : false,
      odAvgCus: formData.isODforAverageCustomer ? true : false,
      cribDocumentId: !formData.isNoCribHistory
        ? uploadData?.id || processInfo?.cribDocumentId
        : null,
    };

    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveDocCheck(request, task.processInstanceId)
    );
    ...
    },
    {
      ...
      content: (
        <form
          ...
          onSubmit={handleSubmit(handleDocCheckSubmission)}
        >
          <div className="grid grid-cols-2 gap-14">
            <div className="flex flex-col gap-14">
              <CribDetailForm
              ...
              />
              <OverDraftCategories editable={true} />
            </div>
            <AttachmentsForm
              documentList={documentList}
              selectedDocument={selectedDocument}
