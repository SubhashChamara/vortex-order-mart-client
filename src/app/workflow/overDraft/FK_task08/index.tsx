...
import { CustomerVerificationRequest } from "../@types/CustomerVerificationRequest";
import { CustomerVerificationInfo } from "../@types/CustomerVerificationInfo";
import FraudCheckForm from "../components/FraudCheckForm/FraudCheckForm";
import { zodResolver } from "@hookform/resolvers/zod";
import { fraudDetectionSchema } from "../schema/Schema";
...
type FormType = {
  ...
  specialVerificationComment: string;

  // fraud detection
  isFraudDetected: boolean | null;
};
...
const defaultValues: FormType = {
  ...
  isNicVerified: false,
  isDobVerified: false,
  specialVerificationComment: "",

  // fraud detection
  isFraudDetected: null,
};
...
const OverDraftFlow: React.FC<OverDraftFlowProps> = ({ task }) => {
  ...
  const [customerVerification, setCustomerVerification] =
    useState<CustomerVerificationInfo | null>(null);
  ...
  const getCustomerVerificationInfo = async () => {
    ...
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getCustomerVerify(task.processInstanceId)
    );
  ...
  // handleFraudDetectionSubmission
  const handleFraudDetectionSubmission = async (formData: FormType) => {
    if (!processInfo) {
      return;
    }
    ...
    const request: OdProcessRequest = {
      ...
      termLoanFacilityType:
        processInfo?.facilityType == "TERM_LOAN"
          ...
          : null,
      fraudDetected: formData.isFraudDetected,
    };

    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveFRMDetails(task.processInstanceId, request)
    );
  ...
  useEffect(() => {
    ...
    if (processInfo) {
      ...
      setValue(
        "isFraudDetected",
        processInfo.fraudDetected === true ? true : false
      );
    }
  ...
  //set customer verification data
  useEffect(() => {
    if (!customerVerification) {
      return;
    }

    setValue("contactPerson", customerVerification.contactName);
    setValue("isAddressVerified", customerVerification.address);
    setValue("isNicVerified", customerVerification.nic);
    setValue("isDobVerified", customerVerification.dob);
    setValue("specialVerificationComment", customerVerification.splComment);
    ...
    },
    {
      ...
      content: (
        <form className="flex flex-col gap-14">
          <div className="grid grid-cols-2 gap-14">
            <div className="flex flex-col gap-14">
              <CribDetailForm editable={false} onUploadButtonClick={() => {}} />
    ...
    },
    {
      ...
      content: (
        <form
          ...
          onSubmit={handleSubmit(handleFraudDetectionSubmission)}
        >
          <div className="grid grid-cols-2 gap-14">
            <div className="flex flex-col gap-14">
              <CustomerVerification editable={false} />
              <FraudCheckForm editable={true} />
            </div>
            <AttachmentsForm
              documentList={documentList}
              selectedDocument={selectedDocument}
