...
import { FrmExpertOpinionActionPointResponse } from "../@types/frmExpertOpinionRequest";

type FrmActionPointProps = {
  frmActionPoint: FrmExpertOpinionActionPointResponse [] | null;
};

const FrmExpertOpinionForm: FC<FrmActionPointProps> = (props) => {
  const { frmActionPoint } = props;
  const [frmInvestigationFlowActionPointList, setfrmInvestigationFlowActionPointList] =
    useState<FrmExpertOpinionActionPointResponse[] | null>(null);

  useEffect(() => {
    if (
      frmActionPoint
    ) {
      setfrmInvestigationFlowActionPointList(
        frmActionPoint
      );
    }
  }, [frmActionPoint]);
      ...
      <div className="col-span-2">
        <TableContainer component={Paper} className="max-w-screen-md">
          <Table className="max-w-screen-md">
            ...
            <TableBody>
              {frmInvestigationFlowActionPointList?.map((actionPoint) => (
                <TableRow>
