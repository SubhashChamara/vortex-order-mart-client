...
import { validateValue } from "../../../core/util/validationUtils";
import { formatInput } from "../../../core/util/formatInput";
...
const FK_Task07: FC<FK_Task07Props> = (props) => {
  ...
  const fetchSelectionCheckListItems = async () => {
    try {
      ...
      if (data !== null) {
        ...
        // console.log("===============> ", selectionCriteriaList);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  const validateItem = (item: CLIProcessCheckListInfo): string | null => {
    if (item.itemType === "txt") {
      return validateValue(item.validationType, item.itemTxtValue || "");
    } else if (item.itemType === "chk") {
      return validateValue(item.validationType, item.itemCheckValue);
    }
    return null;
  };
  ...
  const handleOnSubmit = async (formData: FormType) => {
    for (const item of verifyItems) {
      ...
      if (errorMessage) {
        console.error(`${errorMessage} : "${item.itemName}"`);
        toast.error(`${errorMessage} : "${item.itemName}"`);
        return;
      }
    }
    for (const item of selectionCriteriaList) {
      const errorMessage = validateItem(item);
      if (errorMessage) {
        console.log(`${errorMessage} : "${item.itemName}"`);
        toast.error(`All selection criterias should be selected`);
        return;
      }
    }
  ...
  const handleFormClick = () => {
    ...
    if (customer == null || customer.trim() == "") {
      toast.error("Please complete the Verification Check list !");
    }
  };
    ...
    <div className="grid grid-cols-1 gap-8">
      <FormProvider {...methods}>
        ...
        >
          ...
          <div className="mt-6" onClick={handleFormClick}>
