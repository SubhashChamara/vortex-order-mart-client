...
import { Title } from "../../../../core/types/Title";
...
const UserForm: FC<UserFormProps> = (props) => {
  ...
  const handleOnSubmit = async (formData: FormType) => {
    ...
    const request: CliRequest = {
      ...
      oldNicNumber: oldNicNumber.trim() != "" ? oldNicNumber : null,
      newNicNumber: newNicNumber.trim() != "" ? newNicNumber : null,
      passport: passport.trim() != "" ? passport : null,
      firstName,
      lastName,
      title: title.name,
      addressLine1,
  ...
  useEffect(() => {
    if (cliProcessData !== null) {
      const title: DropDownItem = titleList.find(
        (title) => title.name === cliProcessData.title
      )!;
      const cardType: DropDownItem = cardTypeList.find(
        (card) => card.name === cliProcessData.cardType
      )!;
      const enhancement: DropDownItem = enhancementTypeList.find(
        (enhancement) => enhancement.name === cliProcessData.enhancementType
      )!;
      const upgradeTo: DropDownItem | null = cliProcessData.isUpgradeRequired
        ? cardTypeList.find((card) => card.name === cliProcessData.upgradeTo)!
        : null;
      const modeType: DropDownItem = requestModesList.find(
        (card) => card.name === cliProcessData.modeType
      )!;
      setValue(
        "oldNicNumber",
      ...
      setValue(
        "isCribAttached",
        cliProcessData.isCribAttached ? cliProcessData.isCribAttached : false
      );
    }
  }, [cliProcessData]);
