...
import FrmVerificationForm from "../components/FrmVerificationInstList";
...
type FK_Task01Props = {
  ...
  const FrmExternalVarification: FC<FK_Task01Props> = (props) => {
    const { task } = props;
    ...
    const methods = useForm<FormType>({
      ...
      useEffect(() => {
        const initializeProcess = async () => {
          ...
          const { data, err } = await Api.performRequest((r) =>
            r.creditCard.saveExternalInstance(task.processInstanceId)
          );
      ...
      },
      {
          ...
          content: <FrmExternalVerificationForm procId={task.processInstanceId} form={getExternalVerif} editable={false} commentArea={false} task={task}/>,
        },
      {
  ...
  export default FrmExternalVarification;