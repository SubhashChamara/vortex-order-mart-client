...
const FK_Task01: FC<FK_Task01Props> = (props) => {
  ...
  const handleFetchFrmProcessInfo = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getInvFrmData(task.processInstanceId)
    );
  ...
  const handleFetchFrmQuestionnaire = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getQuestionnaire(task.processInstanceId)
    );
  ...
  const handleFetchActionPoints = async (processInstance: string) => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getActionPoints(processInstance)
    );
  ...
  const handleFetchExpertOpinions = async (processInstance: string) => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getExpertOpinions(processInstance)
    );
  ...
  const handleVerifications = async (processInstance: string) => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getVerification(processInstance)
    );
  ...
  const handleExternalVerifications = async (processInstance: string) => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getFrmExtVeri(processInstance)
    );
  ...
  useEffect(() => {
    const initializeProcess = async () => {
      ...
      const { data, err } = await Api.performRequest((r) =>
        r.creditCard.saveFrmDataEntryPI(task.processInstanceId)
      );
  ...
  },
  {
    ...
    content: <FrmDataEntryForm task={task} dataEntry={dataentry} />
  },
  {
  ...
  useEffect(() => {
    ...
    [form, actionPoints, expertOpinions, verifications, dataentry, externalVeri, frmexpertUsers])
