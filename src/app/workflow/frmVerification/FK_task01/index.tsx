...
type FormType = {
  ...
  const FrmVerification: FC<FK_Task01Props> = (props) => {
    const { task } = props;
    ...
    const methods = useForm<FormType>({
      ...
      useEffect(() => {
        const initializeProcess = async () => {
          ...
          const { data, err } = await Api.performRequest((r) =>
            r.creditCard.saveFrmVerificInstance(task.processInstanceId)
          );
      ...
      },
      {
        ...
        content: <FrmVerificationForm procId={task.processInstanceId} form={getVerificationProctDetails} editable={false} commentArea={false} task={task}/>,
      },
      {
  ...
  useEffect(() => {
   ...
   [getFrmInvestDetails,getVerificationProctDetails,getVerifListOld,getExternalVerif,getQuestion,getDataEntry,getActionPointDetails,getVerifListOld,getExpertOpini,frmexpertUsers,monthList,])
...
export default FrmVerification;