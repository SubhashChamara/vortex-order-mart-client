...
import { salesReworkSchema } from "../schema/Schema";
import { FraudStatusInfo } from "../@types/FraudStatusInfo";
...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  const [fraudStatusInformation, setFraudStatusInformation] =
    useState<FraudStatusInfo | null>(null);
  ...
  const getFraudStatus = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getFraudStatus(task.processInstanceId)
    );

    if (data !== null) {
      setFraudStatusInformation(data);
    } else {
      console.log(err);
    }
  };
  ...
  return (
    <FormProvider {...methods}>
      {fraudStatusInformation?.fraudSelected && (
        <Alert
          variant="outlined"
          color={"error"}
        ...
        >
          <div>
            <p>SUSPECTED FRAUD</p>
            <p className="font-light">{fraudStatusInformation?.fraudComment}</p>
          </div>
        </Alert>
      )}
      {cribPullInformation &&
      ...
      >
        <Ve3StepWizard
          selectStep={steps.findIndex(
            (item) =>
              item.label === "CC Data Entry" || item.label === "Send to Cell"
          )}
          currentSteps={steps
            .slice(
              steps.findIndex(
                (step) =>
                  step.label === "CC Data Entry" ||
                  step.label === "Send to Cell"
              )
            )
            .map(
              (_, index) =>
