...
import { CCDataEntryRequest } from "../@types/CCDataEntryRequest";
...
const schema = z
  .object({
    ...
    supRef: z
      ...
      .optional()
      .refine((val) => !val || val.length === 11 || val.length === 16, {
        message: "Sup Ref must be either 11 or 16 characters long",
      }),
  })
  .refine((data) => data.ecapRef || data.supRef, {
    message: "Either ecapRef or supRef must be provided",
    path: ["ecapRef", "supRef"], // Highlight both fields in error messages
  });
...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  const getStaticDataChangeStatus = async () => {
    ...
    if (data !== null) {
      setValue("isStaticDataChanged", data);
    } else {
      console.log(err);
    }
  };
  ...
  // handle method --> saveStaticDataChangedStatus
  const handleSaveStaticDataChangedStatus = async (formData: FormType) => {
    const { isStaticDataChanged } = formData;
    try {
      const { err } = await Api.performRequest((r) =>
        r.creditCard.saveStaticDataChangedStatus(
          task.processInstanceId,
          task.taskInstance,
          isStaticDataChanged
        )
      ...
      if (!err) {
        toast.success("Static data changed status saved");
      } else {
        toast.error(err.msg);
      }
    } catch (err) {
      ...
      handleCreditCardDataEntry(formData);
    }
  };

  // handle method --> credit card data entry
  const handleCreditCardDataEntry = async (formData: FormType) => {
    const request: CCDataEntryRequest = {
      ecapRef: formData.ecapRef === "" ? null : formData.ecapRef,
      supRef: formData.supRef === "" ? null : formData.supRef,
    };

    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveReferenceCodes(
        task.processInstanceId,
        task.taskInstance,
        request
      )
    ...
    if (!err) {
      toast.success("Credit card data entry saved successfully");
    } else {
      toast.error(err.msg);
    }
  };
  ...
  // useEffect(() => {
  //   if (eyeBallingPreCheckInfo) {
  //     setValue("preCheckNicOld", eyeBallingPreCheckInfo?.nicOld || "");
  //     setValue("preCheckNicNew", eyeBallingPreCheckInfo?.nicNew || "");
  //     setValue("preCheckDob", eyeBallingPreCheckInfo?.dob || "");
  //     setValue(
  //       "preCheckCustomerCategory",
  //       eyeBallingPreCheckInfo?.customerCategory || ""
  //     );
  //     setValue("preCheckName", eyeBallingPreCheckInfo?.name || "");
  //     setValue("preCheckAge", eyeBallingPreCheckInfo?.age || "");
  //   }
  // }, [eyeBallingPreCheckInfo]);
  ...
  const steps = [
      ...
      : []),
    {
      label: "CC Data Entry",
      content: (
        <form onSubmit={handleSubmit(handleCreditCardDataEntry)} noValidate>
          <div className="grid grid-cols-3 gap-12">
            {/* <div className="col-span-3">
              <EyeBallingPreCheckView task={task} />
            </div> */}
            {/* <div className="col-span-1">
              <StaticDataChanges editable={true} />
            </div> */}
  ...
  return (
    <FormProvider {...methods}>
      ...
      <div className="flex flex-col gap-12">
        <Ve3StepWizard
          selectStep={steps.findIndex((step) => step.label === "CC Data Entry")}
          // currentSteps={steps
          //   .slice(steps.findIndex((step) => step.label === "CC Data Entry"))
          //   .map(
          //     (_, index) =>
          //       steps.findIndex((step) => step.label === "CC Data Entry") +
          //       index
          //   )}
          completedSteps={Array.from(
            {
              length:
                steps.findIndex((step) => step.label === "CC Data Entry") + 1,
            },
            (_, index) => index
          )}
          steps={steps}
