...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  // handle method --> save loan type
  const saveDocCheckLoanType = async (formData: FormType) => {
    const request: LoanDocCheckRequest = {
      loanType: formData.loanApplicationType
        ? formData.loanApplicationType?.id.toString()
        : null,
    };

    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveDocCheckLoanType(
        task.processInstanceId,
        task.taskInstance,
        request
      )
  ...
  const steps = [
    ...
    },
    {
      label: "Loan Checks",
      content: (
        <form onSubmit={handleSubmit(saveDocCheckLoanType)}>
          <div className="grid grid-cols-3 gap-12">
            <DocCheckLoanType
              editable={true}
