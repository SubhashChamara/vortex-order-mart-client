...
import { LoanDataEntryInfo } from "../@types/LoanDataEntryInfo";
import { LoanDataEntryRequest } from "../@types/LoanDataEntryRequest";
...
import { loanDataEntrySchema } from "../schema/Schema";
...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  const methods = useForm<FormType>({
    mode: "onChange",
    defaultValues,
    resolver: zodResolver(loanDataEntrySchema),
  });
  ...
  const getLoanDataEntryInfo = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getLoanDataEntryInfo(task.processInstanceId)
    );
  ...
  // handle method --> handle loan data entry submit
  const handleLoanDataEntrySubmit = async (formData: FormType) => {
    console.log("interest rate => ", formData.loanApplicationInterestRate);
    const request: LoanDataEntryRequest = {
      applicantNIC:
        (formData.loanApplicantNIC && formData.loanApplicantNIC) || null,
      applicantName:
        (formData.loanApplicantName && formData.loanApplicantName) || null,
      designation:
        (formData.loanApplicantDesignation &&
          formData.loanApplicantDesignation) ||
        null,
      resAddress1:
        (formData.loanApplicantResAddress1 &&
          formData.loanApplicantResAddress1) ||
        null,
      resAddress2:
        (formData.loanApplicantResAddress2 &&
          formData.loanApplicantResAddress2) ||
        null,
      resAddress3:
        (formData.loanApplicantResAddress3 &&
          formData.loanApplicantResAddress3) ||
        null,
      teleMobile:
        (formData.loanApplicantMobile && formData.loanApplicantMobile) || null,
      teleResident:
        (formData.loanApplicantResTel && formData.loanApplicantResTel) || null,
      companyName:
        (formData.loanApplicantCompany && formData.loanApplicantCompany) ||
        null,
      natureOfBusiness:
        (formData.loanApplicantNatureOfBusiness &&
          formData.loanApplicantNatureOfBusiness) ||
        null,
      salesChannel:
        (formData.loanApplicantSalesChannel &&
          formData.loanApplicantSalesChannel) ||
        null,
      requestedDateTime: dayjs(formData.loanApplicationReceivedDate).format(
        "YYYY-MM-DDTHH:mm:ss"
      ),
      pwidSourceCode:
        (formData.loanApplicationPWIDSC && formData.loanApplicationPWIDSC) ||
        null,
      requestAmount:
        (formData.loanApplicationTotalValue &&
          parseFloat(
            formData.loanApplicationTotalValue.replace(/[^0-9.]/g, "")
          )) ||
        null,
      interestRate:
        (formData.loanApplicationInterestRate &&
          parseFloat(
            formData.loanApplicationInterestRate.replace(/[^0-9.]/g, "")
          )) ||
        null,
      tenor:
        (formData.loanApplicationTenor && formData.loanApplicationTenor) ||
        null,
      masterNo:
        (formData.loanApplicationMasterNo &&
          formData.loanApplicationMasterNo) ||
        null,
      loanRepaymentMode:
        (formData.loanApplicationRepaymentMode &&
          formData.loanApplicationRepaymentMode) ||
        null,
      grossIncome:
        (formData.loanApplicationGrossIncome &&
          parseFloat(
            formData.loanApplicationGrossIncome.replace(/[^0-9.]/g, "")
          )) ||
        null,
      netIncome:
        (formData.loanApplicationNetIncome &&
          parseFloat(
            formData.loanApplicationNetIncome.replace(/[^0-9.]/g, "")
          )) ||
        null,
      dbr:
        (formData.loanApplicationDBR &&
          parseFloat(formData.loanApplicationDBR.replace(/[^0-9.]/g, ""))) ||
        null,
      plLoanType:
        (formData.loanApplicationType &&
          formData.loanApplicationType.id.toString()) ||
        null,
    };

    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveLoanDataEntryInfo(task.processInstanceId, request)
    );
    ...
    if (loanDataEntryInfo) {
      ...
      setValue(
        ...
        (loanDataEntryInfo.interestRate &&
          loanDataEntryInfo.interestRate.toString()) ||
          ""
      );
  ...
  const steps = [
    ...
    ...(isAlpl
      ...
      : []),
    ...(isCC &&
    task.taskId !== "loandataentrycribpass" &&
    task.taskId !== "loandataentryauthcribpass"
      ? [
          {
    ...
    ...(task.taskId === "authoriseapplicationatbranchcasa"
      ...
      : []),
    ...(task.taskName === "Please Update Loan Data Entry" ||
    task.taskName === "Please Authorise Loan Data Entry" ||
    task.taskName === "Please Change Loan Data Entry" ||
    task.taskName === "Please Authorise Loan Data Entry Change"
      ? // task.taskName === "Please Authorise Application at Branch"
        [
          {
            ...
            content: (
              <form
                ...
                onSubmit={handleSubmit(handleLoanDataEntrySubmit)}
                noValidate
              >
                <div className="grid grid-cols-3 gap-12">
                  <div className="col-span-1">
                    <LoanPersonalInformation
                      editable={
                        task.taskName === "Please Authorise Loan Data Entry" ||
                        task.taskId === "loandataentryauthunder"
                          ? false
                          : true
                      }
                    />
                ...
                {(task.taskName === "Please Update Loan Data Entry" ||
                  ...
                  <div className="flex justify-end">
                    <Button type="submit">
                      <EdgeSvgIcon className="pr-3">feather:save</EdgeSvgIcon>
                      Save
                    </Button>
                  </div>
                )}
      ...
      <div className="flex flex-col gap-12">
        <Ve3StepWizard
          selectStep={
            task.taskId === "loandataentrycribpass" ||
            task.taskId === "loandataentryauthcribpass" ||
            task.taskId === "loandataentryunder" ||
            task.taskId === "loandataentryauthunder"
              ? steps.findIndex((item) => item.label === "Loan Data Entry")
              : 0
          }
          currentSteps={
            task.taskId === "loandataentrycribpass" ||
          ...
          completedSteps={
            ...
            task.taskId === "loandataentryauthunder"
              ? Array.from(
                  {
                    length: steps.findIndex(
                      (step) => step.label === "Loan Data Entry"
                    ),
                  },
                  (_, index) => index
                )
              ...
              ? Array.from(
                  {
                    length:
                      steps.findIndex((step) => step.label === "CRIB Details") +
                      1,
                  },
                  (_, index) => index
                )
              : Array.from(
                  {
                    length:
                      steps.findIndex((step) => step.label === "Sales Info") +
                      1,
                  },
                  (_, index) => index
                )
