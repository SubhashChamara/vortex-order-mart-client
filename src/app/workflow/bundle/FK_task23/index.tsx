...
import { creditInformationSchema } from "../schema/Schema";
...
import RequestApprovalInfo from "../components/RequestApprovalInfo/RequestApprovalInfo";
import { ApprovalAmountInfo } from "../@types/ApprovalAmountInfo";
...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  const [reqApprovalAmountInfo, setReqApprovalAmountInfo] =
    useState<ApprovalAmountInfo | null>(null);
  ...
  // get req approval amounts
  const getAmounts = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getAmounts(task.processInstanceId)
    );

    if (data !== null) {
      setReqApprovalAmountInfo(data);
    } else {
      console.log(err);
    }
  };
  ...
  useEffect(() => {
    if (reqApprovalAmountInfo) {
      setValue(
        "reqAmount",
        (reqApprovalAmountInfo.requestAmount &&
          formatCurrency(reqApprovalAmountInfo.requestAmount.toString())) ||
          ""
      );
      setValue(
        "approvedAmount",
        (reqApprovalAmountInfo.approvedAmount &&
          formatCurrency(reqApprovalAmountInfo.approvedAmount.toString())) ||
          ""
      );
    }
  }, [reqApprovalAmountInfo]);
  ...
  const steps = [
    ...
    {
      label: "Request & Approval Amounts",
      content: (
        <div className="grid grid-cols-3">
          <RequestApprovalInfo editable={false} />
        </div>
      ),
    },
  ];
      ...
      >
        <Ve3StepWizard
          selectStep={steps.findIndex(
            (step) => step.label === "Request & Approval Amounts"
          )}
          currentSteps={steps
            .slice(
              steps.findIndex(
