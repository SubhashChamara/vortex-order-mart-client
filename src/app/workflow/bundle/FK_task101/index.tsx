...
import ChequebookList from "../components/ChequebookList/ChequebookList";
import { ChequeBookRequest } from "../@types/ChequeBookRequest";
import { ChequeBookInfo } from "../@types/ChequeBookInfo";
import FDStatus from "../components/FDSpecialStatus/FDStatus";
import { Alert, Button } from "@mui/material";
import EdgeSvgIcon from "../../../../@core/ui/EdgeSvgIcon";
import IBankingForm from "../components/IBankingForm/IBankingForm";
import { IBankingRequest } from "../@types/IBankingRequest";
import { IBankingInfo } from "../@types/IBankingInfo";
...
type FormType = {
  ...
  // cheque book applicant
  chequeApplicantName: string;
  chequeBookAccountNumber: string;
  residentialAddress: string;
  numberOfLeaves: string;
  numberOfBooks: string;
  chequeBookType: DropDownItem | null;

  // fd checks
  FDSpecialStatus: boolean;

  // iBanking checks
  iBankingBeneficiary: boolean;
...
const defaultValues: FormType = {
  ...
  // fd checks
  FDSpecialStatus: false,

  // iBanking checks
  iBankingBeneficiary: false,
  iBankingInternal: false,
  iBankingExternal: false,
};
...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  const [bookTypeDropdowns, setBookTypeCompanyDropdowns] = useState<
    DropDownItem[]
  >([]);
  ...
  const [fdSpecialRate, setFdSpeacialRate] = useState<boolean>(false);
  ...
  const [accountOpenChecklist, setAccountOpenChecklist] = useState<
    ChecklistInfo[]
  >([]);
  const [chequeBookData, setChequeBookData] = useState<ChequeBookInfo[] | null>(
    null
  );

  const [iBankingData, setIBankingData] = useState<IBankingInfo | null>(null);
  ...
  // task name filter
  const determineTaskName = (taskId: string) => {
    if (
      taskId === "pleaseactionempls" ||
      taskId === "pleaseautoemplsfundstransfer" ||
      taskId === "plscheckcommentandaction2" ||
      taskId === "pleasemakechangeschequebookissue2" ||
      taskId === "plsauthocreatechecquebook2" ||
      taskId === "plscreatechecquebook2"
    ) {
      return "Cheque book list";
    }
    return "";
  };
  ...
  // get chequebook list
  const getChequebookEntries = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getChequeBookEntries(task.processInstanceId)
    );

    if (data !== null) {
      setChequeBookData(data);
    } else {
      console.log(err);
    }
  };

  // get fd special rate value
  const getFDSpecialRateValue = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getFDSpecialRate(task.processInstanceId)
    );
    if (data !== null) {
      setFdSpeacialRate(data);
    } else {
      console.log(err?.msg);
    }
  };
  ...
  // get I banking data
  const getIBankingData = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getIBankingData(task.processInstanceId)
    );

    if (data !== null) {
      setIBankingData(data);
    } else {
      console.log(err);
    }
  };

  const addNewChequeBookEntry = async (formData: FormType) => {
    const request: ChequeBookRequest = {
      customerName: formData.chequeApplicantName,
      accountNumber: formData.chequeBookAccountNumber,
      address: formData.residentialAddress,
      numberOfPages: parseInt(formData.numberOfLeaves),
      numberOfBooks: parseInt(formData.numberOfBooks),
      chequeBookType: formData.chequeBookType?.id.toString() || "",
    };
    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveChequeBookEntry(task.processInstanceId, request)
    );

    if (!err) {
      toast.success("Cheque book entry added");
      getChequebookEntries();
    } else {
      toast.error(err.msg);
    }
  };

  const handleChequebookDelete = async (id: number) => {
    const { err } = await Api.performRequest((r) =>
      r.creditCard.deleteCheckbookEntry(task.processInstanceId, id)
    );

    if (!err) {
      toast.success("Cheque book entry deleted");
      getChequebookEntries();
    } else {
      console.log(err);
    }
  ...
  const handleFDSpeacialSubmission = async (formData: FormType) => {
    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveFDSpecialRate(
        task.processInstanceId,
        formData.FDSpecialStatus
      )
    );
    if (!err) {
      toast.success("FD special rate saved");
    } else {
      toast.error(err.msg);
    }
  };

  const handleIBankingSubmission = async (formData: FormType) => {
    const request: IBankingRequest = {
      ibankingBenificiary: formData.iBankingBeneficiary,
      ibankingInternal: formData.iBankingInternal,
      ibankingExternal: formData.iBankingExternal,
    };

    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveIBankingData(task.processInstanceId, request)
    );
    if (!err) {
      toast.success("I Banking request saved");
    } else {
      toast.error(err.msg);
    }
  };
  ...
  useEffect(() => {
    ...
    getDropdownData("cheque-book", setBookTypeCompanyDropdowns);
  ...
  useEffect(() => {
    if (clientInformation) {
      ...
      setValue(
        "chequeApplicantName",
        `${clientInformation.applicantName} ${
          clientInformation.applicantLastName || ""
        }` || ""
      );
  ...
  useEffect(() => {
    if (fdSpecialRate) {
      setValue("FDSpecialStatus", fdSpecialRate);
    }
  }, [fdSpecialRate]);
  ...
  useEffect(() => {
    if (iBankingData) {
      setValue(
        "iBankingBeneficiary",
        iBankingData.ibankingBenificiary || false
      );
      setValue("iBankingInternal", iBankingData.ibankingInternal || false);
      setValue("iBankingExternal", iBankingData.ibankingExternal || false);
    }
  }, [iBankingData]);

  const chequeBookTaskIds = [
    "pleaseactionempls",
    "pleaseautoemplsfundstransfer",
    "plscheckcommentandaction2",
    "pleasemakechangeschequebookissue2",
    "plsauthocreatechecquebook2",
    "plscreatechecquebook2",
  ];

  const fdTaskIds = ["plsauthomasterandrelationship"];
  ...
  const steps = [
    ...
    ...(chequeBookTaskIds.includes(task.taskId)
      ? [
          {
            label: "Cheque book list",
            content:
              task.taskId === "pleaseactionempls" ||
              task.taskId === "pleasemakechangeschequebookissue2" ||
              task.taskId === "plscreatechecquebook2" ? (
                <form onSubmit={handleSubmit(addNewChequeBookEntry)}>
                  <ChequebookList
                    editable={true}
                    bookTypeDropdowns={bookTypeDropdowns}
                    chequeBookData={chequeBookData}
                    handleChequebookDelete={handleChequebookDelete}
                  />
                </form>
              ) : (
                <ChequebookList
      ...
      : []),
    ...(fdTaskIds.includes(task.taskId)
      ? [
          {
            label: "Fixed Deposit",
            content:
              task.taskId === "plsauthomasterandrelationship" ? (
                <form onSubmit={handleSubmit(handleFDSpeacialSubmission)}>
                  <div className="flex flex-col">
                    <FDStatus editable={true} />
      ...
      : []),
    {
      label: "I Banking",
      content: (
        <form onSubmit={handleSubmit(handleIBankingSubmission)}>
          <IBankingForm editable={true} />
          <div className="flex justify-end pt-12">
            <Button type="submit">
              <EdgeSvgIcon>feather:save</EdgeSvgIcon>
              Save
