...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  const handleAccountOpeningSubmission = async () => {
    const isSignatureTicked = accountOpenChecklist
      .find((heading) =>
        heading.checkListItems.find(
          (checklistItem) =>
            checklistItem.masterCheckListItemInfo.name ===
            "Signature to be scanned"
        )
      )
      ?.checkListItems.find(
        (checklistItem) =>
    ...
    if (!isSignatureTicked) {
      toast.error("Signature to be scanned is required");
      return;
    }
  ...
  const steps = [
    ...
    // },
    {
      ...
      content: (
        <form onSubmit={handleSubmit(handleAccountOpeningSubmission)}>
          <AccountOpening
            editable={true}
      ...
      <div className="flex flex-col gap-12">
        <Ve3StepWizard
          selectStep={steps.findIndex(
            (step) => step.label === "Account Opening"
          )}
          currentSteps={steps
            .slice(steps.findIndex((step) => step.label === "Account Opening"))
            .map(
              (_, index) =>
                steps.findIndex((step) => step.label === "Account Opening") +
                index
            )}
