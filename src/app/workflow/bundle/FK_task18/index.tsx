...
import ExternalLetterView from "../components/ExternalLetterView/ExternalLetterView";
...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  const [externalVerificationChecklist, setExternalVerificationChecklist] =
    useState<ChecklistInfo[]>([]);
  ...
  // get external Verifications checklist
  const getExternalVerificationsChecklist = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getBundledChecklistItems(
        task.processInstanceId,
        task.processDefinitionKey,
    ...
    if (data !== null) {
      setExternalVerificationChecklist(data);
    } else {
      console.log(err);
    }
  };
  ...
  // handle method --> external verification checks save
  const handleExternalVerificationCheckSubmission = async () => {
    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveBundleChecklist(
    ...
    if (!err) {
      toast.success("External verification checklist saved");
      // setValue("currentStep", 7);
    } else {
      toast.error(err.msg);
    }
  ...
  useEffect(() => {
    ...
    getExternalVerificationsChecklist();
    getALPLChecklist();
  }, []);
  ...
  useEffect(() => {
    if (uwLevelInformation) {
      ...
      setValue("ccApprovalStatus", uwLevelInformation.cardRejected || false);
  ...
  const steps = [
    ...
    // },
    {
      ...
      content: (
        <form
          onSubmit={handleSubmit(handleExternalVerificationCheckSubmission)}
        >
          <div className="grid grid-cols-3 gap-12">
            {/* <div className="col-span-1">
              <ExternalVerificationChecklist />
            ...
            <div className="col-span-2">
              <ExternalLetterView
                externalVerificationChecklist={externalVerificationChecklist}
      ...
      <div className="flex flex-col gap-12">
        <Ve3StepWizard
          selectStep={steps.findIndex(
            (step) => step.label === "External Verifications"
          )}
          currentSteps={steps
            .slice(
              steps.findIndex((step) => step.label === "External Verifications")
            )
            .map(
              (_, index) =>
                steps.findIndex(
