...
import { LoanDocCheckRequest } from "../@types/LoanDocCheckRequest";
...
import DocCheckLoanType from "../components/DocCheckLoanType/DocCheckLoanType";
...
import AttachAutobook from "../components/AttachAutobook/AttachAutobook";
import EdgeSvgIcon from "../../../../@core/ui/EdgeSvgIcon";
import { AutobookRequest } from "../@types/AutobookRequest";
...
const BundleForm: React.FC<BundledFlowProps> = ({ task }) => {
  ...
  const [loanTypeInfo, setLoanTypeInfo] = useState<LoanDocCheckRequest | null>(
    null
  );

  const [autobookInfo, setAutobookInfo] = useState<AutobookRequest | null>(
    null
  );
  ...
  // get doc check loan type
  const getDocCheckLoanType = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getDocCheckLoanType(task.processInstanceId)
    );

    if (data !== null) {
      setLoanTypeInfo(data);
    } else {
      console.log(err);
    }
  };

  // get auto book attachment status
  const getAutoBookStatus = async () => {
    const { data, err } = await Api.performRequest((r) =>
      r.creditCard.getAutoBookStatus(task.processInstanceId)
    );
    if (data !== null) {
      setAutobookInfo(data);
      // setValue("isAutobookAttached", data);
    } else {
      console.log(err);
    }
  };
  ...
  const handleUWLevelSubmit = async (formData: FormType) => {
    const request: BundleUWLevelRequest = {
      cardRejected:
        formData.ccApprovalStatus === true
          ? false
          : formData.ccApprovalStatus === false
          ? true
          : null,

      loanRejected:
        formData.loanApprovalStatus === true
          ? false
          : formData.loanApprovalStatus === false
          ? true
          : null,
      approverLevelCard:
        ...
        null,
      approverLevelLoan:
        (formData.loanApproverLevel &&
          formData.loanApproverLevel?.id.toString()) ||
        null,
      underwriterCapCard: formData.ccApprovalCap
        ? parseFloat(formData.ccApprovalCap.replace(/[^0-9.]/g, "") || "")
        : null,
      underwriterCapLoan: formData.loanApprovalCap
        ? parseFloat(formData.loanApprovalCap.replace(/[^0-9.]/g, "") || "")
        : null,
      approvedAmount:
        (formData.loanApprovalLimit &&
          parseFloat(
            formData.loanApprovalLimit.replace(/[^0-9.]/g, "") || ""
          )) ||
        null,
      netAmountALPL:
        (formData.loanNetAmount &&
          parseFloat(formData.loanNetAmount.replace(/[^0-9.]/g, "") || "")) ||
        null,
      approvalAttached: !!formData.isLoanApprovalAttached,
      underWriterLimit:
  ...
  // handle method --> save Auto Book Status
  const saveAutoBookStatus = async (formData: FormType) => {
    const request: AutobookRequest = {
      autoBookAttached: formData.isAutobookAttached,
    };
    const { err } = await Api.performRequest((r) =>
      r.creditCard.saveAutoBookStatus(task.processInstanceId, request)
    );

    if (!err) {
      toast.success("Autobook attachment status saved");
    }
  };
  ...
  useEffect(() => {
    if (loanTypeInfo) {
      const matchingLoanType = loanTypeDropdowns.find(
        (item) => item.id.toString() === loanTypeInfo.loanType
      );

      if (matchingLoanType) {
        setValue("loanApplicationType", matchingLoanType);
      }
    }
  }, [loanTypeInfo, loanTypeDropdowns]);

  useEffect(() => {
    if (autobookInfo) {
      setValue(
        "isAutobookAttached",
        (autobookInfo.autoBookAttached && autobookInfo.autoBookAttached) ||
          false
      );
    }
  }, [autobookInfo]);
  ...
  const steps = [
    ...
    },
    {
      label: "Loan Checks",
      content: (
        <form onSubmit={handleSubmit(saveAutoBookStatus)}>
          <div className="grid grid-cols-3 gap-12">
            <DocCheckLoanType
              editable={false}
    ...
    <FormProvider {...methods}>
      ...
      <div className="flex flex-col gap-12">
        <Ve3StepWizard
          selectStep={
            currentStep ||
            steps.findIndex((step) => step.label === "Loan Checks")
          }
          currentSteps={steps
            .slice(steps.findIndex((step) => step.label === "Loan Checks"))
            .map(
              (_, index) =>
                steps.findIndex((step) => step.label === "Loan Checks") + index
            )}
          completedSteps={Array.from(
            {
              length: steps.findIndex((step) => step.label === "Loan Checks"),
            },
            (_, index) => index
          )}
          steps={steps}
