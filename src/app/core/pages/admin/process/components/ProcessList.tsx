...
import RetriveFile from "../../../../../../@helpers/RetriveFiles.ts";
import { darken } from "@mui/material/styles";
import { formatDateAndTime } from "../../helpers/helper-functions";

interface ProcessTableProps {
  processList: ProcessInfo[];
  selectProcess: (ProcessInfo: ProcessInfo) => void;
  deleteProcess: (ProcessInfo: ProcessInfo) => void;
}

const ProcessList: FC<ProcessTableProps> = (props) => {
  const { processList, selectProcess, deleteProcess } = props;
        ...
        <table className="w-full whitespace-no-wrap">
          <thead>
            <tr className="text-xs font-semibold tracking-wide text-left text-black uppercase border-b border-red-600 ">
              <th></th>
              <th className="p-6">Process Name</th>
          ...
          <tbody className="bg-white divide-y divide-red-600">
            {processList.map((process, index) => (
              <tr className="text-black h-full" key={index}>
                ...
                <td className="p-6 align-middle text-sm">{process.name}</td>
                <td className="p-6 align-middle text-sm">{process.description}</td>
                <td className="p-6 align-middle text-sm">{formatDateAndTime(process.createdDate?.toString())}</td>
